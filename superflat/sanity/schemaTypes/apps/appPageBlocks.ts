...
export const appBlockFeatureSection = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'boolean',
      initialValue: true,
      description: 'Turn off to hide the button even when CTA fields are filled.',
    }),
    defineField({
      name: 'showHeadingIcon',
      title: 'Show icon above heading',
      type: 'boolean',
      initialValue: false,
      description: 'Same icon set as Feature grid cards.',
    }),
    defineField({
      name: 'headingIcon',
      title: 'Heading icon',
      ...
      hidden: ({parent}) => !parent?.showHeadingIcon,
    }),
    defineField({name: 'cta', title: 'CTA', type: 'appsCta'}),
  ],
...
export const appBlockWorksWith = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      options: {
        list: [
          {title: 'Text labels', value: 'text'},
          {title: 'SVG icons (upload per tile)', value: 'icons'},
        ...
        defineArrayMember({
          ...
          fields: [
            defineField({
              ...
              type: 'string',
              validation: (Rule) => Rule.required(),
              description: 'Shown for accessibility and as fallback label.',
            }),
            ...
            defineField({
              ...
              description:
                'When Display is “SVG icons”, upload one SVG per tile. Ignored for other modes.',
            }),
            defineField({
              name: 'logo',
              title: 'Logo image',
              type: 'image',
              options: {hotspot: true},
              description: 'Used when Display is “Image squares”. Optional legacy raster logo in row modes.',
            }),
            defineField({
              name: 'logoAlt',
              title: 'Logo alt',
              type: 'string',
              description: 'Alt text for the logo image when used.',
            }),
          ],
          preview: {
            select: {n: 'name'},
